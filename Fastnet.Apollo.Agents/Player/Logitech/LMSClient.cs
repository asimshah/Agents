...
namespace Fastnet.Apollo.Agents
{
    public class LMSClient : LMSApiClient
    {
        ...
        public LMSClient(MusicPlayerOptions mpo, ILogger logger) : base(mpo, logger)
        {
        ...
        public async Task<List<LogitechDevice>> ServerInformationAsync()
        {
            string json = @"{""id"":1,""method"":""slim.request"",""params"":["""",[""serverstatus"",0,999]]}";
            List<LogitechDevice> players = new List<LogitechDevice>();
            try
            {
                ...
                if (root?.Info.PlayerList != null)
                {
                    foreach (var item in root.Info.PlayerList)
                    {
                        var rd = item.ToJson();
                        log.Trace($"item: {rd}");
                        players.Add(new LogitechDevice
                        {
        ...
        public async Task<LogitechDeviceStatus> PlayerInformation(string macAddress)
        {
            try
            {
                ...
                if (droot != null)
                {
                    ...
                    LogitechDeviceStatus status = null;
                    try
                    {
